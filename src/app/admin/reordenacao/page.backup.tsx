          ...
          <div className="bg-[#12131a] border border-white/10 rounded-lg max-w-2xl w-full flex flex-col max-h-[85vh]">
            ...
            <div className="p-6 overflow-y-auto flex-grow flex flex-col gap-6">
              ...
              <div className="bg-white/5 p-4 rounded-lg border border-white/10 flex flex-col gap-4">
                ...
                <div className="form-group mb-0">
                  <label className="form-label text-xs">Arquivo (Anexo)</label>
                  <div className="flex gap-2">
                    <input 
                      type="file" 
                      className="form-input text-sm flex-grow file:mr-4 file:py-1 file:px-3 file:rounded-md file:border-0 file:text-xs file:font-semibold file:bg-[#C1FF07] file:text-black hover:file:bg-[#aee600] cursor-pointer" 
                      onChange={e => {
                        const file = e.target.files?.[0];
                        if (file) {
                          // Simula o upload salvando um caminho fictício com o nome do arquivo original
                          setNewResourceData({...newResourceData, file_url: `/uploads/${file.name}`});
                        }
                      }} 
                    />
