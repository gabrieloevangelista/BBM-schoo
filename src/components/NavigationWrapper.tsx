...
export default function NavigationWrapper({ children }: { children: React.ReactNode }) {
  ...
  const menuItems = [
    ...
    { label: 'Feed', path: '/comunidade', icon: MessageSquare, role: 'all' },
  ...
  return (
    <div className={`flex min-h-screen relative ${theme === 'light' ? 'bg-[#f0f2f5] text-[#1a1a1a]' : 'bg-[#12131a] text-white'}`}>
      ...
      >
        ...
        <nav className="flex-grow py-5 px-3.5 flex flex-col gap-4 overflow-y-auto scrollbar-none">
          ...
          <div className="flex flex-col gap-1.5">
            {filteredItems.filter(item => ['Painel', 'Feed', 'Masterclasses', 'Recursos', 'Calendário', 'Missões', 'Oportunidades', 'Projetos'].includes(item.label)).map(item => {
      ...
      <nav className={`fixed bottom-0 left-0 w-full h-16 backdrop-blur-xl z-50 flex justify-around items-center px-2.5 md:hidden ${theme === 'light' ? 'bg-white/95 border-t border-black/8' : 'bg-[#12131a]/96 border-t border-white/5'}`}>
        ...
        <Link href="/masterclasses" className={`flex flex-col items-center justify-center text-text-secondary hover:text-white no-underline text-[10px] font-outfit font-medium gap-1 flex-grow h-full ${pathname.startsWith('/masterclasses') ? 'text-primary-lemon' : ''}`}>
          <GraduationCap size={20} />
          <span>Aulas</span>
        </Link>
        <Link href="/calendario" className={`flex flex-col items-center justify-center text-text-secondary hover:text-white no-underline text-[10px] font-outfit font-medium gap-1 flex-grow h-full ${pathname === '/calendario' ? 'text-primary-lemon' : ''}`}>
          <Calendar size={20} />
          <span>Agenda</span>
        </Link>
        <Link href="/comunidade" className={`flex flex-col items-center justify-center text-text-secondary hover:text-white no-underline text-[10px] font-outfit font-medium gap-1 flex-grow h-full ${pathname === '/comunidade' ? 'text-primary-lemon' : ''}`}>
          <MessageSquare size={20} />
          <span>Social</span>
        </Link>
        <Link href={`/perfil/${user.username}`} className={`flex flex-col items-center justify-center text-text-secondary hover:text-white no-underline text-[10px] font-outfit font-medium gap-1 flex-grow h-full ${pathname.startsWith('/perfil') ? 'text-primary-lemon' : ''}`}>
          <User size={20} />
          <span>Perfil</span>
        </Link>
      </nav>
